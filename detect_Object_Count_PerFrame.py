...
def count(founded_classes, im0):
    align_top = 30  # Initial vertical position (top)
    align_left = 10  # Horizontal position (left)
    ...
    for i, (k, v) in enumerate(founded_classes.items()):
        ...
        cv2.putText(
            im0,
            text,
            (align_left, align_top + i * 30),  # place each line 30px below the previous
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if view_img:
                ...
                cv2.resizeWindow(window_name, 1300, 1000)  # set window size (change to your screen resolution)
