...
OUTPUT_DIR = "runs/detect/TensorRT Output/Output1"
...
def main():
    ...
    print("[INFO] Generating heatmap with CentroidTracker...")
    ...
    while True:
        ...
        people_boxes = postprocess(detections, frame.shape)
        tracked_objects = tracker.update(people_boxes)

        for objectID, (cx, cy) in tracked_objects.items():
            if 0 <= cx < width and 0 <= cy < height:
                heatmap[cy, cx] += 3  # Increase intensity
    ...
    out_path = str(Path(OUTPUT_DIR) / "centroid_heatmap_overlay.png")
    cv2.imwrite(out_path, overlay)
    print(f"[DONE] Heatmap saved: {out_path}")
